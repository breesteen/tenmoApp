...
public class JdbcUserDao implements UserDao {
    ...
    @Override
    public List<User> findOtherUsers (Integer excludeId) {
        String sql = "SELECT user_id, username FROM tenmo_user WHERE user_id != ?";
        SqlRowSet results = jdbcTemplate.queryForRowSet(sql, excludeId);
        List<User> otherUsers = new ArrayList<>();
        while (results.next()) {
            otherUsers.add(mapOtherUsertoUser(results));
        }
        return otherUsers;
    }
    ...
    private User mapOtherUsertoUser(SqlRowSet rs){
        User user = new User();
